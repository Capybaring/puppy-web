<?php
/** Account navigation with the existing WooCommerce endpoints intact. */
...
<nav class="woocommerce-MyAccount-navigation ipet-account-navigation" aria-label="<?php esc_attr_e('Account pages', 'woocommerce'); ?>">
    <div class="ipet-account-profile">
        <div class="ipet-account-avatar"><?php echo get_avatar($customer->ID, 64, '', $name); ?></div>
        <div class="ipet-account-profile-copy">
            <strong><?php echo esc_html(sprintf(__('Hi, %s', 'puppy-market'), $name)); ?></strong>
            <span><?php echo esc_html($customer->user_email); ?></span>
            <small><?php esc_html_e('iPet customer', 'puppy-market'); ?></small>
        </div>
    </div>
        ...
        <?php foreach ($menu_items as $endpoint => $label) : ?>
            <li class="<?php echo esc_attr(wc_get_account_menu_item_classes($endpoint)); ?>">
                <a href="<?php echo esc_url(wc_get_account_endpoint_url($endpoint)); ?>" <?php echo wc_is_current_account_menu_item($endpoint) ? 'aria-current="page"' : ''; ?>>
                    <span class="ipet-account-menu-icon"><?php echo puppy_market_account_icon($endpoint); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
